...
send() {
  ...
  curl -s -X POST "${BACKEND}/api/ingest" \
    ...
    -d "{
      ...
      \"source\": \"wecom\",
      \"session_id\": \"${SID}_${TS}\",
      \"meta\": {\"from_name\": \"${NAME}\", \"user_id\": \"${SID}_${TS}\", \"recipient\": \"${NAME}\", \"app\": \"企业微信\"},
...
send "user_meta_001" "oscar" "你好，我家人的健康问题也可以问嘛" &

# 场景2: 体检报告 → health → 应选 checkup-report-assistant
send "user_ck_001" "oscar" "我刚拿到体检报告，空腹血糖6.8，总胆固醇5.9，请帮我解读一下" &

# 场景3: 营养饮食 → health → 应选 AI营养师
send "user_nt_001" "oscar" "我最近想减肥，请帮我制定一个适合糖尿病患者的饮食方案" &

# 场景4: 普通闲聊 → chat，直接回复
send "user_chat_001" "oscar" "你好，请问你们周末有客服吗" &
