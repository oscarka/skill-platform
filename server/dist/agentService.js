...
async function createAgentTask(opts) {
    try {
       ...
       VALUES (?, ?, ?, ?, ?, 'pending', ?)`, [opts.id, opts.sessionId, opts.userId, opts.sourceChannel,
...
async function updateAgentTask(id, fields) {
    try {
        ...
        if (fields.routeType !== undefined) {
            sets.push('route_type=?');
            vals.push(fields.routeType);
        }
        if (fields.skillId !== undefined) {
            sets.push('skill_id=?');
            vals.push(fields.skillId);
        }
        if (fields.replyContent !== undefined) {
            sets.push('reply_content=?');
            vals.push(fields.replyContent);
        }
        ...
        if (fields.endedAt !== undefined) {
            sets.push('ended_at=?');
            vals.push(fields.endedAt);
        }
        if (fields.durationMs !== undefined) {
            sets.push('duration_ms=?');
            vals.push(fields.durationMs);
        }
        if (fields.jobTranscript !== undefined) {
            sets.push('job_transcript=?');
            vals.push(fields.jobTranscript);
        }
        if (fields.contextSnapshot !== undefined) {
            sets.push('context_snapshot=?');
            vals.push(fields.contextSnapshot);
        }
        if (fields.cuaEvents !== undefined) {
            sets.push('cua_events=?');
            vals.push(fields.cuaEvents);
        }
        if (!sets.length)
            return;
        vals.push(id);
 ...
 */
async function writeWikiLog(userId, content, type = 'wechat', title) {
    if (!LLMWIKI_BASE || !userId) {
 ...
 */
async function queryContextSnapshot(sessionId, userId) {
    ...
    if (userId) {
        recentTicket = await db.getAsync(`SELECT t.*, tr.raw_result, tr.report_url
       FROM tickets t
       LEFT JOIN ticket_results tr ON tr.ticket_id = t.id
       WHERE t.created_by=? AND t.created_at>? AND t.status != 'error'
       ORDER BY t.created_at DESC LIMIT 1`, [userId, sevenDaysAgo]).catch(() => null);
    }
...
{"skill_id": "xxx或null", "skill_name": "xxx或null", "confidence": "high或low或none", "reason": "一句话理由"}`;
    ...
    try {
        ...
        const confidence = ['high', 'low', 'none'].includes(parsed.confidence)
...
function assembleAgentContext(params) {
    const { req, routeSkillId, routeSkillName, routeSkillDesc, routeConf, guardStatus, guardSkillName, ticketUrl, recentTicket, serviceUrl } = params;
    // 组装 existingTicket
...
async function handleHealthDirect(req, apiKey, requestId, delivery, profile, skillRouteLog) {
    ...
    const directive = req._directive;
...
async function handleHealthSkill(req, apiKey, requestId, delivery, profile, skillId, skillName, skillRouteLog, serviceUrl) {
    ...
    if (isExternal) {
        ...
        const h5BaseRow = await db.getAsync('SELECT value FROM settings WHERE key=?', ['h5_base_url']);
        const h5Base = h5BaseRow?.value || `${serviceUrl}/h5`;
        if (meta.user_id) {
            ...
            if (existing && existing.status !== 'error' && existing.status !== 'expired') {
                ...
                if (reply) {
                    ...
                    void updateAgentTask(requestId, {
                        ...
                        durationMs: endMs - (req._taskStartMs || endMs),
                    });
                    return { request_id: requestId, status: 'done', reply, delivery, route_type: 'ticket_reused' };
                }
    ...
    try {
        if (isApproved && sandboxServiceUrl) {
            ...
            await (0, sandboxServiceClient_1.submitToSandboxService)(sandboxServiceUrl, {
                skillId,
...
只输出 JSON，不输出任何其他文字：`;
    ...
    const forcedSkillId = req.skill_id || null;
    let forcedSkillName = null;
    if (forcedSkillId) {
        const found = availableSkills.find(s => s.id === forcedSkillId)
            || await db.getAsync('SELECT id, name FROM skills WHERE id=?', [forcedSkillId]);
        forcedSkillName = found?.name || forcedSkillId;
    ...
    if (routeConfidence === 'none' && currentGuardStatus !== 'pending_unclear') {
        ...
        // 抢占检查
        const newerTaskChat = await db.getAsync(`SELECT id FROM agent_tasks WHERE session_id = ? AND id != ? AND started_at > ? ORDER BY started_at DESC LIMIT 1`, [req.session_id, requestId, taskStartMs]).catch(() => null);
        if (newerTaskChat) {
    ...
    if (selectedSkillId && selectedSkillName) {
        ...
        void appendTaskEvent(requestId, 'skill_started', {
            skillId: selectedSkillId,
            skillName: selectedSkillName,
            description: skillDesc?.description?.slice(0, 200) || '',
            context_summary: [
        ...
        const skillResult = await handleHealthSkill(req, apiKey, requestId, delivery, profile, selectedSkillId, selectedSkillName, skillRouteLog, serviceUrl);
        ...
        if (agentCtxPkg.directive) {
            req._directive = agentCtxPkg.directive;
