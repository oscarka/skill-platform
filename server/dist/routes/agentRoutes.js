...
async function resolveIdentity(channel, channel_uid, from_name, conversation_id) {
    ...
    const new_unified_id = channel === 'juhe'
        ? `juhe_${channel_uid}` // juhe 先用这个占位符
        : `wecom_${channel_uid}`;
...
async function sendReply(opts) {
    ...
    if (CUA_SEND_URL) {
        const cuaBody = {
            ...
            reply,
            // delivery.recipient 必须是显示名，CUA 需要用它在页面搜索客户
...
exports.agentRouter.post('/ingest', async (req, res) => {
    try {
        ...
        const agentReq = {
            ...
            session_id: unified_id, // 统一 session（unified_id 替换原始 from_user_id）
            meta: {
                from_name: display_name,
                user_id: unified_id,
                channel_uid: from_user_id, // 保留原始渠道 ID 佛备查
            },
            context: {
                available_apps: ['企业微信'],
        ...
        (0, agentService_1.processAgentChat)(agentReq).then(result => {
            console.log(`[Orch/Ingest] done unified=${unified_id} status=${result.status} reply="${(result.reply || '').slice(0, 60)}"`);
            if (result.reply) {
                sendReply({
                    reply: result.reply,
                    juhe_conv_id,
...
exports.agentRouter.delete('/debug/guards', async (req, res) => {
    ...
    try {
        const now = Date.now();
        const result = await db.runAsync(`UPDATE skill_confirm_guards SET status='closed', updated_at=? WHERE user_id=? AND status='active'`, [now, userId]);
