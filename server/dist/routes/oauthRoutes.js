...
exports.oauthRouter.get('/api/oauth/google/config', (req, res) => {
    if (!GOOGLE_CLIENT_ID) {
...
exports.oauthRouter.get('/auth/google/start', (req, res) => {
    const sessionId = ensureSessionId(req, res);
    const skillId = req.query.skill_id || '';
    const scope = req.query.scope || 'https://www.googleapis.com/auth/userinfo.email';
...
exports.oauthRouter.get('/auth/google/callback', async (req, res) => {
    try {
        ...
        const expiresAt = tokenData.expires_in
            ...
            : null;
        await db.runAsync(`INSERT INTO oauth_tokens (id, session_id, provider, skill_id, access_token, refresh_token, token_type, expires_at, scope)
       VALUES (?, ?, 'google', ?, ?, ?, ?, ?, ?)`, [
            (0, crypto_1.randomUUID)(), sessionId, skillId || null,
...
exports.oauthRouter.get('/api/oauth/tokens', async (req, res) => {
    try {
        const sessionId = ensureSessionId(req, res);
        const tokens = await db.allAsync(`SELECT id, provider, skill_id, token_type, scope, expires_at, created_at
       FROM oauth_tokens WHERE session_id=? ORDER BY created_at DESC`, [sessionId]);
        res.json({ tokens });
    }
    catch (err) {
...
exports.oauthRouter.delete('/api/oauth/tokens/:id', async (req, res) => {
    try {
        const sessionId = ensureSessionId(req, res);
...
exports.oauthRouter.get('/api/oauth/token-for-skill', async (req, res) => {
    try {
        const skillId = req.query.skill_id;
        if (!skillId)
            return res.status(400).json({ error: 'skill_id required' });
        // 查找与该 skill 关联的最新 token（不限 session）
        const token = await db.getAsync(`SELECT access_token, refresh_token, token_type, expires_at
       FROM oauth_tokens WHERE (skill_id=? OR skill_id IS NULL) AND provider='google'
       ORDER BY created_at DESC LIMIT 1`, [skillId]);
        if (!token)
...
exports.oauthRouter.post('/api/oauth/mcp-tokens', async (req, res) => {
    try {
        ...
        await db.runAsync(`INSERT INTO mcp_oauth_tokens (id, provider, mcp_name, access_token, refresh_token, expires_at, token_data, created_at, updated_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
       ON CONFLICT (provider, mcp_name) DO UPDATE SET
         access_token=$4, refresh_token=$5, expires_at=$6, token_data=$7, updated_at=$9`, [id, provider, mcp_name || null, access_token, refresh_token || null, expires_at, token_data ? JSON.stringify(token_data) : null, now, now]);
        res.json({ ok: true, id });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
exports.oauthRouter.get('/api/oauth/mcp-tokens', async (req, res) => {
    try {
        const rows = await db.allAsync(`SELECT id, provider, mcp_name, expires_at, updated_at,
              CASE WHEN expires_at = 0 OR expires_at > $1 THEN 'valid' ELSE 'expired' END as status
       FROM mcp_oauth_tokens ORDER BY updated_at DESC`, [Date.now()]);
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
exports.oauthRouter.delete('/api/oauth/mcp-tokens/:id', async (req, res) => {
    try {
