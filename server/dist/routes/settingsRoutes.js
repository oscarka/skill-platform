...
exports.settingsRouter.get('/', async (_req, res) => {
    try {
        const rows = await db.allAsync('SELECT key, value, updated_at FROM settings ORDER BY key');
        // Mask API keys
...
exports.settingsRouter.put('/', async (req, res) => {
    try {
        ...
        for (const { key, value } of updates) {
            ...
            await db.runAsync('INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, ?)', [key, value, Date.now()]);
        }
 ...
 */
exports.settingsRouter.get('/models', (_req, res) => {
    res.json({
        models: [
            ...
            { id: 'deepseek-v4-flash', provider: 'deepseek', label: 'DeepSeek V4 Flash 0731 ⚡' },
 ...
 */
exports.settingsRouter.post('/test-key', async (req, res) => {
