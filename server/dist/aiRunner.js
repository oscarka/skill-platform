...
function inferProvider(modelName) {
    ...
    if (modelName.includes('flash') || modelName.includes('ga-') || modelName.startsWith('deepseek-v4') || modelName.startsWith('deepseek-v3-') || modelName.startsWith('deepseek-r1-'))
        return 'doubao';
...
// ─── Gemini 调用 ──────────────────────────────────────────────────────────────
async function runGemini(prompt, model, apiKey, opts) {
...
async function runOpenAICompat(prompt, model, apiKey, baseURL, opts) {
    ...
    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`AI request timed out after ${timeoutMs / 1000}s`)), timeoutMs));
...
async function runAI(prompt, opts = {}) {
    ...
    const tryRun = async (model) => {
        ...
        if (p === 'gemini') {
            ...
            if (!config.doubaoKey)
                throw new Error('Doubao API key not configured. Please set it in Settings.');
