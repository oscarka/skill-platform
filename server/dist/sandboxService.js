 ...
 */
async function runSandboxTest(skillId, oauthTokens, caseCount = 1, manualTestInput, attachmentGcsPaths = [], overrideModel) {
    const t0 = Date.now();
    // 如果没有显式传入 token，从 DB 自动取存储的 MCP OAuth token
    if (!oauthTokens) {
        try {
            const storedTokens = await db.allAsync(`SELECT provider, mcp_name, access_token, token_data, expires_at FROM mcp_oauth_tokens WHERE expires_at = 0 OR expires_at > $1`, [Date.now()]);
    ...
    // 标记为 running
    await db.runAsync(`UPDATE skills SET sandbox_status='running', updated_at=? WHERE id=?`, [Date.now(), skillId]);
    try {
        const skill = await db.getAsync('SELECT * FROM skills WHERE id=?', [skillId]);
        if (!skill)
            throw new Error('Skill not found');
        // 获取 SKILL.md 内容（存在 prompt_template 里）
        const content = skill.prompt_template || skill.code || '';
        if (!content)
            throw new Error('Skill has no content (prompt_template is empty)');
        const parsed = (0, skillParser_1.parseSkillMd)(content);
        // ── 路由决策（两层保险）─────────────────────────────────────────────────
        ...
        // 只要任一信号认为需要沙箱，就进沙箱
        const dbSkillType = skill.skill_type || 'prompt';
        const needsSandbox = parsed.skillType === 'script' ||
            dbSkillType === 'plugin' ||
            dbSkillType === 'code';
        ...
        let result;
        if (needsSandbox) {
            if (cloudRunJobsClient_1.USE_CLOUD_RUN) {
        ...
        // 写入结果
        await db.runAsync(`UPDATE skills SET sandbox_status='done', sandbox_test=?, updated_at=? WHERE id=?`, [JSON.stringify(result), Date.now(), skillId]);
    }
    catch (err) {
        ...
        await db.runAsync(`UPDATE skills SET sandbox_status='failed', sandbox_test=?, updated_at=? WHERE id=?`, [JSON.stringify(errResult), Date.now(), skillId]);
    }
}
// ─── Prompt 类：3轮 ReAct Loop ───────────────────────────────────────────────
async function runPromptReActLoop(parsed, t0) {
    const trace = [];
...
只输出 JSON，不要其他内容。`;
    ...
    try {
        const r0 = await (0, aiRunner_1.runAI)(genInputPrompt, { model, temperature: 0.7 });
        usedModel = r0.model;
    ...
    let round1Output = '';
    try {
        const r1 = await (0, aiRunner_1.runAI)(round1Prompt, {
            model,
...
如果输出需要改进，请输出改进版本。如果已经足够好，输出"[已完整]"开头然后重申要点。`;
    let round2Output = '';
    try {
        const r2 = await (0, aiRunner_1.runAI)(selfReviewPrompt, { model, temperature: 0.2, maxTokens: 2048 });
...
只输出 JSON，不要其他内容。`;
    ...
    let strengths = ['内容相关', '格式清晰'];
    let weaknesses = [];
    try {
        const r3 = await (0, aiRunner_1.runAI)(evalPrompt, { model, temperature: 0.1, maxTokens: 512 });
 ...
 */
function reflectOnError(cmd, result) {
    if (result.exitCode === 0 && !result.timedOut)
        return null; // 成功，不需要 Reflect
    const errText = (result.stderr + result.stdout).toLowerCase();
    // 精确匹配"命令不存在"，排除"文件不存在"（scripts/ 找不到是另一个问题）
    const isCmdNotFound = errText.includes('command not found') ||
 ...
 */
function dockerExec(containerId, cmd) {
    return new Promise((resolve) => {
        ...
        const child = (0, child_process_1.exec)(fullCmd, { timeout });
 ...
 */
async function runDockerReActLoop(skill, parsed, t0) {
    const trace = [];
    let containerId = '';
    let usedModel = 'default';
    ...
    try {
        // ── 检查 Docker 镜像是否存在 ────────────────────────────────────────────
        try {
            (0, child_process_1.execSync)(`docker image inspect ${SANDBOX_IMAGE} --format '{{.Id}}'`, { stdio: 'pipe' });
        }
        catch {
            throw new Error(`Docker 镜像 ${SANDBOX_IMAGE} 不存在，请先运行 docker build`);
        }
        // ── 启动一次性临时沙箱容器 ────────────────────────────────────────────────
        const containerName = `skill-sandbox-${(0, crypto_1.randomUUID)().slice(0, 8)}`;
        ...
        containerId = (0, child_process_1.execSync)(`docker run -d --name ${containerName} \
       --memory=512m --cpus=1.0 \
       --network=bridge \
       --rm \
       ${SANDBOX_IMAGE} tail -f /dev/null`, { encoding: 'utf8' }).trim();
        ...
        (0, child_process_1.execSync)(`docker exec -u root ${containerId} bash -c "mkdir -p /workspace/skill && chown -R sandbox:sandbox /workspace"`, { stdio: 'pipe' });
        ...
        if (parsed.hasScripts) {
            const scriptsPath = skill.scripts_path;
            if (scriptsPath) {
                // 通道 1：直接从宿主机路径复制
                trace.push({ round: 0, role: 'think', content: `📦 从本地路径加载 scripts/: ${scriptsPath}` });
                try {
                    (0, child_process_1.execSync)(`docker cp ${scriptsPath}/. ${containerId}:/workspace/skill/`, { stdio: 'pipe' });
                ...
                try {
                    const { execSync: execCheck } = await Promise.resolve().then(() => __importStar(require('child_process')));
                    // 先检查是否已安装
                    let skillDir = '';
                    try {
                        (0, child_process_1.execSync)(`test -d ${ocWorkspaceSkillPath}`, { stdio: 'pipe' });
                        ...
                        (0, child_process_1.execSync)(`node ${ocMjs} skills install ${parsed.slug} 2>/dev/null`, {
                    ...
                    (0, child_process_1.execSync)(`docker cp ${skillDir}/. ${containerId}:/workspace/skill/`, { stdio: 'pipe' });
                    // chown 给 sandbox 用户（docker cp 默认以 root 身份写入）
                    (0, child_process_1.execSync)(`docker exec -u root ${containerId} chown -R sandbox:sandbox /workspace/skill`, { stdio: 'pipe' });
...
请分析这个技能，然后输出以下 JSON（只输出 JSON，不要其他内容）：
{
  ...
  "reasoning": "为什么从这条命令开始"
                ...
                const r0 = await (0, aiRunner_1.runAI)(planPrompt, { temperature: 0.3 });
                usedModel = r0.model;
                const jsonMatch = r0.text.match(/\{[\s\S]*\}/);
                if (jsonMatch) {
...
- 最多 ${MAX_ROUNDS} 轮，请聚焦核心功能验证`;
        ...
        for (let round = 1; round <= MAX_ROUNDS && !loopDone; round++) {
            ...
            if (round < MAX_ROUNDS) {
                try {
                    const aiResp = await (0, aiRunner_1.runAI)(messages[messages.length - 1].content, {
...
只输出 JSON。`;
        let score = 60, passed = false, comment = '测试完成', strengths = [], weaknesses = [];
        try {
            const evalResp = await (0, aiRunner_1.runAI)(evalPrompt, { temperature: 0.1, maxTokens: 512 });
        ...
        if (containerId) {
            try {
                (0, child_process_1.execSync)(`docker stop ${containerId} 2>/dev/null || docker kill ${containerId} 2>/dev/null || true`, { stdio: 'pipe' });
...
// ─── 进度追加工具 ─────────────────────────────────────────────────────────────
async function appendProgress(skillId, event) {
    try {
        const row = await db.getAsync('SELECT sandbox_progress FROM skills WHERE id=?', [skillId]);
        const events = row?.sandbox_progress ? JSON.parse(row.sandbox_progress) : [];
        events.push({ ts: new Date().toISOString(), ...event });
        await db.runAsync(`UPDATE skills SET sandbox_progress=?, updated_at=? WHERE id=?`, [JSON.stringify(events), Date.now(), skillId]);
    }
    catch (err) {
        // 不能静默吸掉，必须记录错误以便调试
...
// ─── Cloud Run Job 测试路径 ───────────────────────────────────────────────────
async function runCloudRunJobTest(skill, parsed, t0, oauthTokens, caseCount = 1, manualTestInput, attachmentGcsPaths = []) {
    const skillId = skill.id;
    const content = skill.prompt_template || skill.code || '';
    // 重置进度
    await db.runAsync(`UPDATE skills SET sandbox_progress=? WHERE id=?`, [JSON.stringify([]), skillId]);
    ...
    let aiKey, aiBase, fallbackKey, fallbackBase;
    if (isGemini) {
...
{"test_case_1": "具体的测试请求1", "test_case_2": "具体的测试请求2"}`;
                const genResp = await (0, aiRunner_1.runAI)(genPrompt, { temperature: 0.7, maxTokens: 300 });
                const jm = genResp.text.match(/\{[\s\S]*\}/);
                if (jm)
                    testInputs = JSON.parse(jm[0]);
    ...
    try {
        ...
        if (skillMcpNames.length > 0) {
            ...
            if (mcpRows.length > 0) {
                ...
                console.log(`[SandboxService] skill ${skill.id} mcp_names=${JSON.stringify(skillMcpNames)} → loaded ${mcpRows.length} configs:`, mcpRows.map((r) => r.name));
            }
        }
        else {
    ...
    const { executionId, executionName } = await (0, cloudRunJobsClient_1.submitSandboxJob)({
        skillId,
    ...
    while (Date.now() < deadline) {
        ...
        const { getExecutionStatus } = await Promise.resolve().then(() => __importStar(require('./cloudRunJobsClient')));
    ...
    // ── 自动保存沙箱中发现的 MCP 配置 ────────────────────────────────────────
    try {
        const progRow = await db.getAsync('SELECT sandbox_progress FROM skills WHERE id=?', [skillId]);
        if (progRow?.sandbox_progress) {
            ...
            for (const e of events) {
                ...
                if (m) {
                    ...
                    try {
                        await db.runAsync(`INSERT INTO mcp_configs (id, name, command, args) VALUES (?, ?, ?, ?)
               ON CONFLICT(name) DO UPDATE SET command=excluded.command, args=excluded.args`, [randomUUID(), name, command, args]);
    ...
    const row = await db.getAsync('SELECT sandbox_test FROM skills WHERE id=?', [skillId]);
    let callbackResult = null;
    try {
        callbackResult = row?.sandbox_test ? JSON.parse(row.sandbox_test) : null;
    }
    catch { }
