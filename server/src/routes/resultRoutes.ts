...
import { triggerWikiSyncPublic, writeWikiLog } from '../agentService';
...
resultRouter.post('/:ticketId/confirm', async (req, res) => {
  try {
    const ticket = await db.getAsync<any>('SELECT * FROM tickets WHERE id=?', [req.params.ticketId]);
    if (!ticket) return res.status(404).json({ error: 'Ticket not found' });
    ...
    const result = await db.getAsync<any>('SELECT * FROM ticket_results WHERE ticket_id=?', [ticket.id]);
    if (!result) return res.status(400).json({ error: 'No result to confirm' });

    const skill = await db.getAsync<any>('SELECT name FROM skills WHERE id=?', [ticket.skill_id]);

    // Update ticket status
    await db.runAsync(
      `UPDATE tickets SET status='patient_confirmed', updated_at=? WHERE id=?`,
      [Date.now(), ticket.id]
    );
...
resultRouter.post('/:ticketId/reject', async (req, res) => {
  try {
    const ticket = await db.getAsync<any>('SELECT id, status FROM tickets WHERE id=?', [req.params.ticketId]);
    if (!ticket) return res.status(404).json({ error: 'Ticket not found' });
    if (ticket.status !== 'done')
    ...
    await db.runAsync(
      `UPDATE tickets SET status='patient_rejected', updated_at=? WHERE id=?`,
      [Date.now(), ticket.id]
    );
