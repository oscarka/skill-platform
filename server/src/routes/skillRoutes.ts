...
export interface SkillRecord {
  ...
  tags?: string;       // JSON array e.g. '["agent版"]' | '[]' | null
}
...
skillRouter.patch('/:id/model', async (req, res) => {
  try {
    const skill = await db.getAsync<SkillRecord>('SELECT * FROM skills WHERE id=?', [req.params.id]);
    if (!skill) return res.status(404).json({ error: 'Skill not found' });
    const { preferred_model, fallback_model } = req.body;
    await db.runAsync(
      `UPDATE skills SET preferred_model=?, fallback_model=COALESCE(?,fallback_model), updated_at=? WHERE id=?`,
      [preferred_model ?? null, fallback_model ?? null, Date.now(), req.params.id]
    );
    const updated = await db.getAsync<SkillRecord>('SELECT * FROM skills WHERE id=?', [req.params.id]);
...
skillRouter.patch('/:id/meta', async (req, res) => {
  try {
    const skill = await db.getAsync<SkillRecord>('SELECT * FROM skills WHERE id=?', [req.params.id]);
    if (!skill) return res.status(404).json({ error: 'Skill not found' });
