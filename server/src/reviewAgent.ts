...
export interface ReviewResult {
  ...
  h5_config_suggestion?: {
    ...
    uploads?: { accept: string[]; maxFiles: number; maxSizeMB: number; label: string };
  };
  turns: number;
  durationMs: number;
}
...
export interface ReviewAgentParams {
  ...
  skillVisibility: 'internal' | 'external';  // internal=员工 | external=客户
...
const READ_SKILL_TOOL: OpenAI.Chat.ChatCompletionTool = {
  type: 'function',
  function: {
    name: 'read_skill_content',
    description:
      '读取指定 Skill 的完整内容（SKILL.md 全文，即 prompt_template）。' +
...
async function buildOpenAIClient(model: string) {
  ...
  if (isDoubao) {
    ...
    if (!apiKey) throw new Error('Doubao API key not configured');
    return new OpenAI({ apiKey, baseURL, timeout: 300_000 });
  } else if (isDeepSeek) {
    ...
    const baseURL = (await getSetting('doubao_base_url', 'DOUBAO_BASE_URL')) ||
      'https://ark.cn-beijing.volces.com/api/v3';
    if (!apiKey) throw new Error('Doubao API key not configured (used as fallback for tool-call review)');
    return new OpenAI({ apiKey, baseURL, timeout: 300_000 });
  }
}
...
async function execReadSkillContent(skillId: string): Promise<string> {
  // 和 sandbox runner.py 的 _fetch_skill_md_from_db() 逻辑对齐
  // search_path 已设为 skill_platform，无需写 schema 前缀
  const row = await db.getAsync<{ prompt_template: string | null }>(
    'SELECT prompt_template FROM skills WHERE id=?',
    [skillId]
  );
  if (!row) throw new Error(`Skill ${skillId} 不存在`);
  if (!row.prompt_template) throw new Error(`Skill ${skillId} 的 prompt_template 为空`);
  return row.prompt_template;
}

// ─── 主入口 ───────────────────────────────────────────────────────────────────

export async function runSkillReviewAgent(params: ReviewAgentParams): Promise<ReviewResult> {
  const startTime = Date.now();
  let turn = 0;

  const log = (step: string, detail: string) => {
    const p: ReviewProgress = { ts: new Date().toISOString(), step, detail };
  ...
  try {
    ...
    if (!jsonMatch) throw new Error('AI 未返回合法 JSON');
